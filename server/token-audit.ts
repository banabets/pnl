...
export class TokenAuditService {
   ...
   */
  async auditToken(mintAddress: string): Promise<TokenAudit> {
    ...
    try {
      ...
      let riskLevel: TokenAudit['riskLevel'] = 'low';
   ...
   */
  async quickCheck(mintAddress: string): Promise<{ safe: boolean; reason?: string }> {
    try {
      const mint = new PublicKey(mintAddress);
      const mintInfo = await this.connection.getParsedAccountInfo(mint);
      const mintData = (mintInfo.value?.data as any)?.parsed?.info;
   ...
   */
  private async getRugcheckData(mintAddress: string): Promise<any> {
    try {
      // Try RugCheck API
      ...
      if (response.ok) {
        ...
        return {
          ...
          topHoldersPercent: data.topHolders?.reduce((sum: number, h: any) => sum + (h.pct || 0), 0) || 0,
   ...
   */
  async isHoneypot(mintAddress: string): Promise<{ isHoneypot: boolean; reason?: string }> {
    try {
      ...
      try {
        ...
        if (response.ok) {
          const data = await response.json();
          if (data.risks?.some((r: any) => r.name?.toLowerCase().includes('honeypot'))) {
...
// Singleton
let auditInstance: TokenAuditService | null = null;

export function getTokenAuditService(rpcUrl?: string): TokenAuditService {
...
export function initTokenAuditService(rpcUrl: string): TokenAuditService {
  auditInstance = new TokenAuditService(rpcUrl);
  return auditInstance;
}
