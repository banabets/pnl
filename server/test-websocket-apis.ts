...
async function testPumpPortalAPI(): Promise<TestResult> {
...
async function testPumpFunSocketIO(): Promise<TestResult> {
  const result: TestResult = {
  ...
  return new Promise((resolve) => {
    ...
    try {
      const socket: Socket = io('https://frontend-api.pump.fun', {
...
async function runComparison() {
  ...
  const winner = results.reduce((best, current) => {
    if (!current.connected) return best;
    if (!best.connected) return current;
    
    // Score based on multiple factors
    const currentScore = 
      (current.tokensReceived * 10) +
      (current.dataQuality === 'high' ? 50 : current.dataQuality === 'medium' ? 25 : 0) -
      (current.latency / 10) -
      (current.errors.length * 20);
    
    const bestScore = 
      (best.tokensReceived * 10) +
