...
export default function VolumeBot({ socket }: VolumeBotProps) {
  ...
  const handleStart = async () => {
    ...
    if (config.walletCount > totalWallets) {
      alert(`You only have ${totalWallets} wallets. Reduce the amount or create more wallets.`);
      return;
    }
    ...
    try {
      ...
      alert('Volume bot started successfully');
      loadStatus();
    } catch (error: any) {
      alert('Error starting volume bot: ' + (error.response?.data?.error || error.message));
    } finally {
      setIsLoading(false);
    }
  };
  ...
  const handleStop = async () => {
    ...
    try {
      ...
      alert('Error stopping volume bot: ' + (error.response?.data?.error || error.message));
    } finally {
      setIsLoading(false);
    }
  };
  ...
  return (
    <div className="space-y-6">
      <div className="bg-black rounded-lg p-6 border border-white/15 shadow-[0_2px_8px_rgba(0,0,0,0.5),inset_0_1px_0_rgba(255,255,255,0.1)]">
        ...
        <p className="text-white/60 text-sm mb-6">Generate automatic trading volume on tokens using multiple wallets</p>

        {/* Status Cards */}
        {status.isRunning && (
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
            <div className="bg-black rounded-lg p-4 border border-white/15">
              <div className="text-white/60 text-xs font-medium uppercase tracking-wider mb-1">Volume Generated</div>
              <div className="text-2xl font-bold text-green-400">{status.currentVolume.toFixed(2)} SOL</div>
              <div className="text-xs text-white/50 mt-1">of {status.targetVolume.toFixed(2)} SOL</div>
            </div>
            <div className="bg-black rounded-lg p-4 border border-white/15">
              <div className="text-white/60 text-xs font-medium uppercase tracking-wider mb-1">Trades Executed</div>
              <div className="text-2xl font-bold text-blue-400">{status.tradesExecuted}</div>
            </div>
            <div className="bg-black rounded-lg p-4 border border-white/15">
              <div className="text-white/60 text-xs font-medium uppercase tracking-wider mb-1">Wallets Used</div>
              <div className="text-2xl font-bold text-purple-400">{status.walletsUsed}</div>
            </div>
            <div className="bg-black rounded-lg p-4 border border-white/15">
              <div className="text-white/60 text-xs font-medium uppercase tracking-wider mb-1">Running Time</div>
        ...
        <div className="space-y-4">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <div>
              <label className="block text-white/80 text-sm font-medium mb-2">
                Number of Wallets ({totalWallets} available)
              </label>
              <input
                type="number"
          ...
          <div className="flex gap-3 pt-4">
            {!status.isRunning ? (
              ...
              >
                {isLoading ? 'Starting...' : '▶ Start Volume Bot'}
              </button>
            ) : (
              <button
                onClick={handleStop}
              ...
              >
                {isLoading ? 'Stopping...' : '⏹ Stop Volume Bot'}
              </button>
            )}
          </div>
        </div>
        ...
        {status.errors.length > 0 && (
          <div className="mt-6 bg-red-500/10 border border-red-500/30 rounded-lg p-4">
            <div className="text-red-400 font-semibold mb-2">⚠️ Errors ({status.errors.length})</div>
        ...
        <div className="mt-6 bg-blue-500/10 border border-blue-500/30 rounded-lg p-4">
          ...
          <ul className="text-sm text-blue-300 space-y-1 list-disc list-inside">
            <li>The bot executes automatic trades to generate volume</li>
            <li>Uses multiple wallets to simulate organic activity</li>
            <li>60% buys / 40% sells for upward pressure</li>
            <li>Trade sizes are randomized between min and max</li>
            <li>Make sure you have enough SOL in the wallets</li>
          </ul>
        </div>
      </div>
    </div>
