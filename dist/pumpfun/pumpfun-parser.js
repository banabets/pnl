...
exports.PumpFunTransactionParser = void 0;
...
class PumpFunTransactionParser {
    constructor(rpcUrl) {
        this.connection = new web3_js_1.Connection(rpcUrl || process.env.RPC_URL || 'https://api.mainnet-beta.solana.com', 'confirmed');
    }
    /**
     * Get trades from pump.fun program transactions
     */
    async getTradesFromPumpFunProgram(tokenMint, limit = 50) {
        try {
            ...
            const signatures = await this.connection.getSignaturesForAddress(programPubkey, { limit: Math.min(limit * 2, 100) }, 'confirmed');
            ...
            for (let i = 0; i < Math.min(signatures.length, 50); i++) {
                try {
                    ...
                    const mintInTx = accounts.some((acc) => acc.pubkey?.toBase58() === tokenMint);
                    if (!mintInTx)
                        continue;
     ...
     */
    async parsePumpFunTransaction(tx, signature, tokenMint) {
        try {
            if (!tx.meta)
                return null;
            ...
            for (const preBalance of preTokenBalances) {
                if (preBalance.mint === tokenMint) {
                    const postBalance = postTokenBalances.find((pb) => pb.accountIndex === preBalance.accountIndex && pb.mint === tokenMint);
                    if (postBalance) {
                        ...
                        if (!accountOwner)
                            accountOwner = preBalance.owner || signerAccount;
                    }
                }
            }
                ...
                if (postBalance.mint === tokenMint) {
                    ...
                    if (!preBalance) {
                        ...
                        if (postAmount > 0) {
                            ...
                            if (!accountOwner)
                                accountOwner = postBalance.owner || signerAccount;
                        }
                    }
                }
            }
     ...
     */
    async getTradesFromBondingCurve(bondingCurveAddress, tokenMint, limit = 50) {
        try {
            const bondingCurvePubkey = new web3_js_1.PublicKey(bondingCurveAddress);
            // Get recent signatures for the bonding curve account
            const signatures = await this.connection.getSignaturesForAddress(bondingCurvePubkey, { limit: Math.min(limit, 50) }, 'confirmed');
            const trades = [];
...
exports.PumpFunTransactionParser = PumpFunTransactionParser;
//# sourceMappingURL=pumpfun-parser.js.map