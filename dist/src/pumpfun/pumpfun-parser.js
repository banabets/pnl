...
class PumpFunTransactionParser {
        ...
        try {
            ...
            for (let i = 0; i < Math.min(signatures.length, 50); i++) {
                try {
                    ...
                    const mintInTx = accounts.some((acc) => (acc.pubkey?.toBase58?.() || acc?.toBase58?.() || acc) === tokenMint);
                    if (!mintInTx)
                        continue;
     ...
     */
    async parsePumpFunTransaction(tx, // Support both ParsedTransactionWithMeta and VersionedTransactionResponse
    signature, tokenMint) {
        try {
            if (!tx.meta)
                return null;
