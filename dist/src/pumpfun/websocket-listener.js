        ...
        try {
            ...
            try {
                recentSignatures = await Promise.race([
                    this.connection.getSignaturesForAddress(PUMP_FUN_PROGRAM_ID, { limit: 20 }, // Reduced from 50 to avoid rate limits
                    'confirmed'),
                ...
                try {
                    ...
                    try {
                        tokenSignatures = await Promise.race([
                            publicRpc.getSignaturesForAddress(spl_token_1.TOKEN_PROGRAM_ID, { limit: 20 }, // Reduced from 50
                            'confirmed'),
                            new Promise((_, reject) => setTimeout(() => reject(new Error('Timeout')), 10000))
                        ]).catch(() => []);
                    }
